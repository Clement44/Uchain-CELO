...
contract("CarbonCredit", function (accounts) {
  ...
  var ipfsCarbonCertificateHash =
    "QmcFULbtwMYLwe2cUdUvvWNQVcRsqKCVgFbaUGgseJcpsa";
  ...
  it("can depositCarbonCreditsFromCertificate", async () => {
    let tx = await token.depositCarbonCreditsFromCertificate(
      100,
    ...
    truffleAssert.eventEmitted(
      ...
      (ev) => {
        assert.equal(
          ev.ifpsHashOfCarbonCreditCertificate,
          ipfsCarbonCertificateHash
        );
        assert.equal(ev.value, 100);
  ...
  it("only owner can deposit carbon credits from certificate", async () => {
    await truffleAssert.reverts(
      token.depositCarbonCreditsFromCertificate(
        100,
        ipfsCarbonCertificateHash,
        {
          from: notOwner,
  ...
  it("can lookup the carbon credit certificates that have been deposited and get their value", async () => {
    await token.depositCarbonCreditsFromCertificate(
      100,
      ipfsCarbonCertificateHash
    );
    assert.equal(await token.carbonCertificates(0), ipfsCarbonCertificateHash);
    assert.equal(
      await token.carbonCertificateValue(ipfsCarbonCertificateHash),
      100
    );
  });
  ...
  it("cannot deposit the same carbon credit more than once", async () => {
    ...
    await truffleAssert.reverts(
      token.depositCarbonCreditsFromCertificate(100, ipfsCarbonCertificateHash),
  ...
  it("considers a 0 value carbon credit to not be set", async () => {
    await truffleAssert.reverts(
      token.depositCarbonCreditsFromCertificate(0, ipfsCarbonCertificateHash),
      "Certificate must have a value greater than 0"
    );
